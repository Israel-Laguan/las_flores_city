...
**Exception — "manual" key:** A few dangerous migrations (currently 095 + 096 + 097) live under the top-level `"manual"` array. They are present in the registry (so every file on disk is accounted for) but are deliberately ignored by the runner. 095 is not production-ready (hard-coded dev passwords, CREATE ROLE assumes CREATEROLE which managed Postgres often lacks); production roles and schemas must be provisioned out of band using environment-specific credentials. 096 is the transition helper for legacy role names. 097 is the permission lockdown that revokes `las_flores` access to the planning/runtime schemas (apply last: 095 → 096 → 097). These must only ever be applied by hand with environment-specific credentials. New entries in "manual" require a comment explaining why they cannot go through the normal path.
