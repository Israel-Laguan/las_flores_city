...
- The shared UI workspace `@las-flores/ui` is the single source of truth for design tokens, theme variables, base CSS, and reusable component classes. Admin imports `tokens.css` + `global.css` + `components.css` from there; the game `client` imports only `themes.css`. The two theme namespaces (`--accent`/`--background` in `tokens.css` vs `--color-*` in `themes.css`) are deliberately separate — do NOT "unify" them in a single PR. Full contract: [docs/UI_STYLE_SYSTEM.md](docs/UI_STYLE_SYSTEM.md).
