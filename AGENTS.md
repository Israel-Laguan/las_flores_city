...
**Exception — "manual" key:** Currently empty (`"manual": []`). Previously 095+096+097 lived here (095 was not production-ready — hard-coded dev passwords, CREATE ROLE assumes CREATEROLE; 096/097 have since been deleted). Production roles/schemas must be provisioned out of band using environment-specific credentials. New entries in "manual" require a comment explaining why they cannot go through the normal path.
