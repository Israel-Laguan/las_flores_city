---
name: Official Statement from the District of Old Las Flores
type: story-illustration
size: 1920x1080
source: content/lore/stories/old_las_flores_district_statement/old_las_flores_district_statement.md
target: `assets/old_las_flores_district_statement__default.png`
consumer: html-background
---

# Prompt: Official Statement from the District of Old Las Flores — reference image

## Prompt — Base Scene
A cinematic comic-book panel, modern colorful graphic novel style. Setting: an outdoor podium on a street corner in a weathered historic district, midday, a crowd on cracked pavement. A district official in a formal shirt and badge reads a statement at a microphone, flanked by two uniformed officers. Behind, a banner with a district seal. Buildings show faded murals, boarded windows, laundry lines; residents listen, skeptical to hopeful. Podium centered. Vibrant colors with grounded realism, DC/Marvel-quality illustration, cinematic lighting, highly detailed, textures of weathered plaster, fabric, and paper.

## Negative Prompt
--no text overlay, watermarks, neon, satellites, androids, robots, cybernetics, futuristic sci-fi, holograms, extreme violence, gore, guns, magic, anime

## Variations
- [ ] Close-up on the printed statement on the podium, the district seal visible, hands of two different residents resting on the edge of the frame
- [ ] Shot from behind the crowd, the official small at the podium, the full neighborhood visible in the background
