...
source: content/characters/ingrid_haugen/ingrid_haugen.prompt.md
target: content/characters/ingrid_haugen/assets/
---

# Video Prompts: Ingrid Haugen

Generate seamless looping portrait videos from each expression variant PNG. Use the corresponding `ingrid_haugen__<expression>.png` as the input image for each prompt. The woman on the reference image is the character described in the source prompt file.
