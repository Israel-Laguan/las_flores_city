...
function build_manifest() {
  local prompt_files
  prompt_files=$(find "${PROMPT_ROOTS[@]}" -name '*.prompt.md' -not -path '*/references/*' 2>/dev/null | sort)

  while IFS= read -r pf; do
    [[ -z "$pf" ]] && continue
    # Output absolute path so downstream can resolve against any root
    local type
...
function cmd_init() {
  ...
  while IFS=$'\t' read -r abs_path variant w h; do
    [[ -z "$abs_path" ]] && continue
    count=$((count+1))
    local base
    base=$(basename "$abs_path" .prompt.md)
    local prompt_dir
    prompt_dir=$(dirname "$abs_path")
    local slug
    slug=$(echo "$variant" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/_/g' | sed 's/^_\|_$//g')
    local draft_path_abs="$prompt_dir/assets/${base}__${slug}.png"
    local rel="$abs_path"
    local size=0
    ...
    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" "$rel" "$variant" "$w" "$h" "$draft_path_abs" "$size" "$status" "$(date -Iseconds)" >> "$STATE_FILE"
  done < <(build_manifest)
...
UNIFIED_SCRIPT="$SCRIPT_DIR/generate-drafts-unified.mjs"

function cmd_run() {
  header "Running Unified Draft Generator (NIM → Pollinations per prompt)"
  echo -e "  ${CYAN}Args:${NC} $*"
...
function cmd_retry() {
  ...
  if [[ -f "$STATE_FILE" ]]; then
    while IFS=$'\t' read -r rel var _w _h dp_abs _size status _ts; do
      [[ "$rel" == "prompt_rel" ]] && continue
      if [[ "$status" == "failed" ]]; then
        [[ -f "$dp_abs" ]] && rm -f "$dp_abs" && echo -e "  ${YELLOW}Cleaned${NC} $(basename "$dp_abs")"
      fi
    done < "$STATE_FILE"
  fi
  cmd_run "$@"
...
function cmd_clean() {
  ...
  done < <(find "${PROMPT_ROOTS[@]}" -path "*/assets/*.png" -type f 2>/dev/null)
