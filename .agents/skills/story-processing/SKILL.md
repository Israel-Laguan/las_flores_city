...
- **No empty-list risk**: every entry node keeps ≥1 choice with no relationship gate, so
  `filterChoices` (fail-closed) can never return zero choices.
...
- **neutral_default** is fail-open and intentional only (e.g. first-time guarded version); it is
  NOT the default and must not be used to paper over a missing relationship row elsewhere.
