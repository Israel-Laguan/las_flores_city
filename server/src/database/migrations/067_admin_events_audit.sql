...
-- CHECK, re-add with the extended allowed set using NOT VALID (so the
-- ADD never scans existing rows / locks writes), and VALIDATE the
-- constraint in a follow-up migration once this one is committed.
