...
async function processLocationData(data: any): Promise<string> {
  ...
  const sceneData = {
    ...
    id: data.id,
    name: data.name,
    description: sanitizeText(data.description || data.history || ''),
    district: data.district || 'Unknown',
    image_url: data.image_url || data.asset_paths?.image || null,
    background_url: data.background_url || data.asset_paths?.background || null,
