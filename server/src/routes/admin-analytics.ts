...
adminAnalyticsRouter.get('/summary', async (_req, res) => {
  try {
    // Dialogue completion rates: dialogues started vs completed
    const dialogueCompletions = await queryOLAP<{
      event_type: string;
    ...
    // Story beat reach: which beats have been set on players
    const storyBeatReach = await queryOLAP<{
      event_type: string;
      story_beat: string;
    ...
    // Total player count for percentage calculations
    const totalPlayers = await queryOLAP<{ count: string }>(`
      SELECT count(DISTINCT user_id)::int AS count
      FROM player_events
    `);
    const playerCount = Number(totalPlayers?.rows[0]?.count ?? 0);

    // Mystery status distribution (from OLTP since mysteries is OLTP)
    const mysteryStatus = await queryOLTP<{ status: string; count: number }>(`
