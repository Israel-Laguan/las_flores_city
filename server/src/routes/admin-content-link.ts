...
function applyLinkAction(data: any, fieldPath: string, action: string, value: string): void {
  ...
  } else if (action === 'remove' && Array.isArray(current)) {
...
adminContentLinkRouter.post('/link', async (req, res) => {
  ...
  try {
    ...
    applyLinkAction(data, fieldPath, action, value);
