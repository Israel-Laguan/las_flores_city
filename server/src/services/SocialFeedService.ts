...
// In-memory fallback cache for graceful degradation when Redis is down.
...
export class SocialFeedService {
  ...
  static async getFeed(limit = 30): Promise<SocialPost[]> {
    ...
    // Secondary path — in-memory fallback if Redis is unavailable.
    ...
    // Refresh in-memory fallback so the next Redis failure is also cushioned
