...
beforeAll(async () => {
  ...
  // 4. Discover chunk IDs + leaf IDs generated by the compiler. The `leaves`
  //    JSONB column is dropped in M32, so leaf IDs are read from the
  //    CDN-loaded resolved chunk instead.
  ...
  for (const row of chunks.rows) {
    if (row.chunk_key === 'chunk_start') {
      startChunkId = row.id;
      const resolved = await DialogueResolver.resolveChunkForUser(TEST_USER_ID, row.id, row.chunk_key);
      for (const [leafId, leaf] of Object.entries(resolved.chunk.leaves)) {
