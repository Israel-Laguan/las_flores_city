...
scope:
  in:
    ...
    - Load source YAML via `admin-content.ts` read path, sanitize per content type: strip `id`, `created_at`, `updated_at`; remove all relationship IDs (e.g., `speaker_id`, `scene_id`, `dialogue_id`); reset asset paths to `<slug>__default.png`; compute new slug from new name
...
approach:
  ...
  - Sanitize: remove `id`, `created_at`, `updated_at`; strip relationship IDs (`speaker_id`, `scene_id`, `dialogue_id`, etc.); reset all asset paths to `<slug>__default.png`; compute new slug from new name
risks:
