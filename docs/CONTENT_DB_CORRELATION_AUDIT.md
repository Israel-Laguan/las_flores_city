...
- character: Mapped via processCharacterData → upsertCharacter. DB table is one row per YAML. Admin detail renders EntityDetailView. ✅
- dialogue: Mapped via processDialogueData → upsertDialogueTree. nodes JSONB is large but functional. modifications/nodes cascade on overlays. ✅
- overlay: Mapped via processOverlayData → upsertDialogueOverlay. Admin detail returns full modifications + nodes. ✅
- scene: Mapped via processSceneData → upsertScene. Extra step links metadata.npcs to scene_characters. ✅
- gig: Mapped via processGigData → upsertGig. Single YAML or gigs: [] array; first id returned as migration key. ✅
- mission (DB table mysteries): Mapped via processMissionData → upsertMystery. migration_log CHECK updated to accept mission; legacy rows renamed from mystery → mission. ✅
- vault: Dir content/vault/ is a single file containing vault_items: []. Parsed as VaultFileSchema and iterated in processVaultData. ✅
- shop_item: Single-file array, parsed with ShopItemFileSchema. Upserted individually. ✅
...
- location (DB table scenes):
  - Upsert: processLocationData (upsert.ts:118-137) spreads the entire YAML including location-only keys into metadata, then calls upsertScene. The district column in scenes is populated from data.district (defaults to Unknown).
  - Drifts:
    - D1: metadata->>type = location is the only discriminator — no first-class table, no FK.
    - D2: available_dialogues is hardcoded to []; no npcs are linked to scene_characters. So the public endpoint returns 0 NPCs for location content.
    - D3: District naming mismatch between seeded DB (Downtown, Old Town, Commercial, Industrial, South, City) vs. authored paths (southeast, rio_de_las_flores, pacific, industrial, port, far_south, central, northeast, south).
    - D6: Schema fields do not match real YAML keys (see §5).
- story_beat: Usable, but stored as slug PK — breaks any code that assumes the Plan 00 resolver id is a UUID.
- map_tile: Table + upsert + admin view exist, but content/maps/ is missing and the table is empty by default. The client shows empty grids.
- district: Not a content type. Districts are created by upsertScene (auto-insert if missing) or by the migration seed files. There is no admin CRUD.
...
### D1 — scene/location conflation

**What:** location rows live in the scenes table with metadata->>type = location, but the public GET /location endpoint (server/src/routes/location.ts:248-296) returns *all* scenes (no filter), while admin /admin/locations returns only the location-tagged subset.

**Evidence:** server/src/routes/location.ts:257-264 JOINs districts but has no metadata->>type=location predicate.

**Impact:** Public /api/location returns scene+location mix; admin /admin/locations returns only locations. The same scenes table serves both roles, making it easy to accidentally overwrite location fields when a scene YAML migrates.

**Recommended fix:** Add AND metadata->>type = location to the public /api/location list, or keep the split explicit in docs. Do not introduce a separate locations table (AGENTS.md existing patterns rule).

### D2 — Locations do not link characters/dialogues

**What:** processSceneData reads metadata.npcs and inserts into scene_characters, but processLocationData hardcodes both available_dialogues: [] and never touches scene_characters.

**Evidence:** server/src/content/upsert.ts:121-133 — available_dialogues: [] is literal; no scene_characters insert.
...
**Recommended fix:** Extend YAMLLocationSchema with optional npcs: uuid[] and available_dialogues: uuid[], and mirror the processSceneData linking after upsertScene. Alternatively, keep locations lore-only and link characters through a companion content/scenes/<slug>/scene_*.yaml per location.

### D3 — districts naming mismatch

**What:** DB seeds define districts named Downtown, Old Town, Commercial, Industrial, South, City, Unknown. Content lives under content/districts/southeast, port, central, northeast, south, industrial, rio_de_las_flores, pacific, far_south, city. Only 2 of 10 overlap.

**Evidence:** server/src/database/migrations/034_seed_districts.sql:13-59. find content/districts -maxdepth 2 -type d shows 9 non-matching slugs.

**Impact:** Most location YAMLs omit district or use district: Unknown because the real district names do not map cleanly. upsertScene auto-creates districts rows for any string it sees (upsert.ts:72-75), so the DB accumulates mismatched district names. The tile map (client MapView) shows the DBs 7 seeded districts, not the 9 authored ones.

**Recommended fix:** Normalize to whichever side needs fewer moves — option (a) rename content/districts/* folders to match seeded slugs, or option (b) replace seeded rows to match authored slugs and add a content/districts mapping doc. Either way, run ./scripts/apply-migrations.sh both after because scenes.district_id FK will need backfill.
...
**Impact:** map_tiles table is empty in a fresh or migrated state → client DistrictView renders an empty tile grid.
...
**Impact:** Authors editing a location map.grid are editing a field that the client never reads. The client never reads it either — no code consumes YAMLLocationSchema.map in the game client.
...
**What:** The schema specifies description, history, daytime, nightlife, conclusion, important_places, map. Real content/districts/.../location_*.yaml files (77 total) use overview, life_and_work, airport_employment, adjacent_industries, community_dynamics. None of the 77 files define daytime or nightlife.

**Evidence:** grep -rl ^map: content/districts --include=*.yaml | wc -l → 1. grep -rl daytime: content/districts --include=*.yaml → 0. Sample southeast.yaml has no description/history/daytime/nightlife. YAMLLocationSchema is not .strict(), so extra keys are silently accepted.

**Impact:** The admin location editor (LOCATION_EDIT_FIELDS) exposes fields that do not exist in content, while hiding fields that do. Migrated DB rows end up with empty/undefined column values because the YAML never supplied them.

**Recommended fix:** Either migrate the authored YAMLs to description/history/daytime/nightlife (and unify overview → history semantics), or extend YAMLLocationSchema to include overview/life_and_work/etc. Renaming authored content has higher blast radius; extending the schema is lower risk.
...
**Impact:** If we follow Phase 4 (extend edit views to all entities) without a special case, GET /admin/content/by-id?type=story_beat&id=<slug> returns 404 or wrong entity.
...
1. **map_tiles is empty.** There is no content/maps/ directory and no migration seed for map_tiles. Until tiles exist with coordinates, MapView.tsx has nothing to render in DistrictView.
...
- (a) Populate map_tiles for at least one district, with metadata.location_id pointing at the corresponding location row.
- (b) Extend location YAML schema + processLocationData to link characters via scene_characters (D2 fix).
