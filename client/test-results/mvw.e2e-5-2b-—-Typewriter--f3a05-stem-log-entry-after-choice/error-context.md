...
- Name: mvw.e2e.test.ts >> 5.2b — Typewriter Skip & Choice Selection >> Monologue console feed appends system log entry after choice
- Location: tests/e2e/mvw.e2e.test.ts:179:3
...
Locator: locator('.introspection-console, #monologue-feed, .monologue-feed').locator('p, .log-entry, .feed-item').first()
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.introspection-console, #monologue-feed, .monologue-feed').locator('p, .log-entry, .feed-item').first()
