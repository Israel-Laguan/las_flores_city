...
var DialogueUIState;
(function (DialogueUIState) {
    DialogueUIState["HIDDEN"] = "HIDDEN";
    DialogueUIState["SLIDING_IN"] = "SLIDING_IN";
    DialogueUIState["TYPING"] = "TYPING";
    DialogueUIState["AWAITING_CHOICE"] = "AWAITING_CHOICE";
    DialogueUIState["SUBMITTING"] = "SUBMITTING";
    DialogueUIState["SLIDING_OUT"] = "SLIDING_OUT";
...
export class DialogueUI {
    ...
    setupStyles() {
        ...
        this.container.style.webkitBackdropFilter = 'blur(10px)';
    ...
    setupEventListeners() {
        eventBus.on('dialogue:start', async (data) => {
        ...
        eventBus.on('dialogue:choose', async (choiceIndex) => {
    ...
    async startDialogue(dialogueId) {
        try {
    ...
    async startDialogueWithCharacter(characterId, sceneId) {
        try {
    ...
    async makeChoice(choiceIndex) {
        if (!this.currentDialogue || this.state === DialogueUIState.SUBMITTING)
            return;
        this.state = DialogueUIState.SUBMITTING;
        this.disableButtons();
        try {
            const result = await api.makeDialogueChoice(this.currentDialogue.tree.id, choiceIndex);
            if (result.success && result.data) {
    ...
    renderDialogue() {
        ...
        this.dialogueTextEl = this.container.querySelector('.dialogue-text');
    ...
    buildDialogueHTML(currentNode, availableChoices) {
    ...
    attachChoiceButtonListeners() {
        ...
        choiceButtons.forEach(button => {
            ...
            button.addEventListener('mouseenter', () => {
                if (!button.disabled) {
                    button.style.backgroundColor = 'rgba(0, 255, 0, 0.15)';
                    button.style.borderColor = 'rgba(0, 255, 0, 0.6)';
                }
            });
            button.addEventListener('mouseleave', () => {
                button.style.backgroundColor = 'rgba(0, 255, 0, 0.05)';
