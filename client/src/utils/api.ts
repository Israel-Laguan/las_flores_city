...
import type { VaultItem, ShopItem, PlayerInventoryItem, PublicProfile } from '../types/api';
...
export async function devLogin(userId?: string): Promise<any> {
  return fetchAPI('/auth/dev-login', { method: 'POST', body: JSON.stringify({ userId }) });
}
...
// Banco API
export async function getBankStatement(): Promise<{ success: boolean; data: BankLedgerResponse; timestamp?: string }> {
...
export async function startDialogue(dialogueId: string): Promise<any> {
  return fetchAPI(`/dialogue/${dialogueId}/start`, { method: 'POST' });
}

export async function startDialogueWithCharacter(characterId: string, sceneId: string): Promise<any> {
...
export async function makeDialogueChoice(dialogueOrChunkId: string, choiceIdOrIndex: string | number): Promise<any> {
  const body: any = {};
  ...
  return fetchAPI(`/dialogue/${dialogueOrChunkId}/choose`, { method: 'POST', body: JSON.stringify(body) });
}
...
export async function endDialogue(): Promise<any> {
  return fetchAPI('/dialogue/end', { method: 'POST' });
}
...
// Fire-and-forget server sync for instant-transition cache hits.
export function makeDialogueChoiceBackground(currentChunkId: string, choiceId: string): void {
  void (async () => {
    ...
    const attempt = () => fetchAPI(endpoint, { method: 'POST', body });

    try {
      await attempt();
    } catch (err) {
      if (!shouldIntercept(err)) return;
      eventBus.emit('ui:show_error', {
        id: typeof crypto !== 'undefined' && 'randomUUID' in crypto
          ? crypto.randomUUID()
          : `err_${Date.now()}_${Math.random().toString(36).slice(2)}`,
        signature: `POST ${endpoint}`,
        code: err instanceof TypeError ? 'UPLINK_BROKEN' : (`SERVER_CRASH_${(err as { status?: number }).status}` as const),
        message: 'Uplink Broken — Retrying…',
        retry: () => attempt(),
        abort: () => {
          eventBus.emit('monologue:push', { text: '[UPLINK ABANDONED] Server sync skipped. Save state may be out of sync.', type: 'warning' });
        },
      });
    }
  })();
...
export async function toggleAiEnabled(enabled: boolean): Promise<any> {
  return fetchAPI('/settings/ai-enabled', { method: 'PATCH', body: JSON.stringify({ enabled }) });
}

export async function updateDisplayName(displayName: string): Promise<any> {
  return fetchAPI('/settings/profile', { method: 'PATCH', body: JSON.stringify({ display_name: displayName }) });
}
...
// Vault API
export type { VaultItem } from '../types/api';
...
export async function fetchVaultMediaUrl(itemId: string): Promise<string> {
  try {
    ...
    if (message.includes('ENTITLEMENT_REVOKED')) throw new VaultMediaError('ENTITLEMENT_REVOKED', message);
    if (message.includes('ACCESS_DENIED_OR_NOT_OWNED')) throw new VaultMediaError('ACCESS_DENIED_OR_NOT_OWNED', message);
...
// Shop / MyMe API
export type { ShopItem, PlayerInventoryItem, PublicProfile } from '../types/api';
...
export async function buyShopItem(shopItemId: string): Promise<{ success: boolean; data: { inventory_item: PlayerInventoryItem; new_balance: number; currency_type: 'credits' | 'gold_credits' }; timestamp?: string }> {
  return fetchAPI('/shop/buy', { method: 'POST', body: JSON.stringify({ shop_item_id: shopItemId }) });
}

export async function equipShopItem(slot: 'theme' | 'border', shopItemId: string | null): Promise<{ success: boolean; data: { slot: string; shop_item_id: string | null }; timestamp?: string }> {
  return fetchAPI('/shop/equip', { method: 'POST', body: JSON.stringify({ slot, shop_item_id: shopItemId }) });
}
...
export async function startPayPalCheckout(amountUsd: number): Promise<{ success: boolean; data: { order_id: string; reference_id: string; approve_url: string }; timestamp?: string }> {
  return fetchAPI('/paypal/checkout', { method: 'POST', body: JSON.stringify({ amount_usd: amountUsd }) });
}