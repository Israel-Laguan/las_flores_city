...
export default function VaultDetailPage() {
    ...
    async function fetchRecord() {
      try {
        ...
        if (res.status === 404) { setNotFound(true); } else {
          ...
          if (data.success) { setRecord(data.data); } else { setError(data.error || 'Failed to fetch vault item'); }
        }
      } catch { setError('Failed to fetch vault item'); } finally { setLoading(false); }
    }
    fetchRecord();
  }, [id]);
  ...
  return (
    <main style={styles.main}>
      <a href="/vault" style={{ color: '#00ff00' }}>← Back to Vault</a>
      <h1 style={styles.heading}>Vault Item: {id}</h1>
