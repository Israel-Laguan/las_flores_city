...
export default function LocationDetailPage() {
    ...
    async function fetchRecord() {
      try {
        ...
        if (res.status === 404) { setNotFound(true); } else {
          ...
          if (data.success) { setRecord(data.data); } else { setError(data.error || 'Failed to fetch location'); }
        }
      } catch { setError('Failed to fetch location'); } finally { setLoading(false); }
    }
    fetchRecord();
  }, [id]);
  ...
  return (
    <main style={styles.main}>
      <a href="/locations" style={{ color: '#00ff00' }}>← Back to Locations</a>
      <h1 style={styles.heading}>Location: {id}</h1>
