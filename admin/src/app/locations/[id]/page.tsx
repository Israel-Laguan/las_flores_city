...
function BackgroundImageCard({ record }: { record: Location }) {
  const src = record.background_url || record.image_url;
  if (!src) return null;
...
function BasicInfoCard({ record }: { record: Location }) {
  return (
    <div style={cardStyle}>
...
function AudioCard({ record }: { record: Location }) {
  return (
    <div style={cardStyle}>
      <div style={labelStyle}>Ambient Sound</div>
      {record.ambient_sound_url ? (
        <audio controls src={record.ambient_sound_url} style={{ width: '100%' }} />
      ) : (
...
function MetadataCard({ record }: { record: Location }) {
...
function LinkedDialoguesCard({ record }: { record: Location }) {
  const dialogues = record.available_dialogues;
    ...
    <div style={cardStyle}>
      ...
      <ul style={{ listStyle: 'none', padding: 0, margin: '0.5rem 0 0 0' }}>
        {dialogues.map((dialogueId) => (
          <li key={dialogueId} style={{ marginBottom: '0.5rem' }}>
            <Link href={`/dialogues/${dialogueId}`} style={{ color: '#00ff00' }}>
              {dialogueId}
            </Link>
          </li>
        ))}
...
function TimestampsCard({ record }: { record: Location }) {
  return (
    <div style={cardStyle}>
...
function RawJson({ record }: { record: Location }) {
  ...
  return (
    <main style={styles.main}>
      ...
      {!loading && !notFound && !error && record && (
        <>
          <BackgroundImageCard record={record} />
          <BasicInfoCard record={record} />
          <AudioCard record={record} />
          <MetadataCard record={record} />
          <LinkedDialoguesCard record={record} />
