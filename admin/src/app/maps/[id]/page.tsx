...
export default function MapTileDetailPage() {
    ...
    async function fetchRecord() {
      try {
        ...
        if (res.status === 404) { setNotFound(true); } else {
          ...
          if (data.success) { setRecord(data.data); } else { setError(data.error || 'Failed to fetch map tile'); }
        }
      } catch { setError('Failed to fetch map tile'); } finally { setLoading(false); }
    }
    fetchRecord();
  }, [id]);
  ...
  return (
    <main style={styles.main}>
      <a href="/maps" style={{ color: '#00ff00' }}>← Back to Maps</a>
      <h1 style={styles.heading}>Map Tile: {id}</h1>
