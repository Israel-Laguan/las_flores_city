...
export default function MysteryDetailPage() {
  ...
  useEffect(() => {
    async function fetchRecord() {
      try {
        const res = await fetch(`/api/admin/mysteries/${id}`);
        if (res.status === 404) { setNotFound(true); } else {
          const data = await res.json();
          if (data.success) { setRecord(data.data); } else { setError(data.error || 'Failed to fetch mystery'); }
        }
      } catch { setError('Failed to fetch mystery'); } finally { setLoading(false); }
    }
    fetchRecord();
  }, [id]);
  ...
  return (
    <main style={styles.main}>
      <a href="/mysteries" style={{ color: '#00ff00' }}>← Back to Mysteries</a>
      <h1 style={styles.heading}>Mystery: {id}</h1>
