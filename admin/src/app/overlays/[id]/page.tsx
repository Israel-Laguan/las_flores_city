...
export default function OverlayDetailPage() {
  ...
  useEffect(() => {
    async function fetchRecord() {
      try {
        const res = await fetch(`/api/admin/overlays/${id}`);
        if (res.status === 404) { setNotFound(true); } else {
          const data = await res.json();
          if (data.success) { setRecord(data.data); } else { setError(data.error || 'Failed to fetch overlay'); }
        }
      } catch { setError('Failed to fetch overlay'); } finally { setLoading(false); }
    }
    fetchRecord();
  }, [id]);
  ...
  return (
    <main style={styles.main}>
      <a href="/overlays" style={{ color: '#00ff00' }}>← Back to Overlays</a>
      <h1 style={styles.heading}>Overlay: {id}</h1>
