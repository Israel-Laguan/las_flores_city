...
export default function ShopDetailPage() {
    ...
    async function fetchRecord() {
      try {
        ...
        if (res.status === 404) { setNotFound(true); } else {
          ...
          if (data.success) { setRecord(data.data); } else { setError(data.error || 'Failed to fetch shop item'); }
        }
      } catch { setError('Failed to fetch shop item'); } finally { setLoading(false); }
    }
    fetchRecord();
  }, [id]);
  ...
  return (
    <main style={styles.main}>
      <a href="/shop" style={{ color: '#00ff00' }}>← Back to Shop</a>
      <h1 style={styles.heading}>Shop Item: {id}</h1>
