...
export default function ScenesPage() {
  ...
  const fetchPage = useCallback(async (p: number) => {
    try {
      const res = await fetch(`/api/admin/scenes?page=${p}&pageSize=${pageSize}`);
          ...
          <table style={styles.table}>
            <thead>
              <tr>
                ...
                <th style={styles.th}>Required Story Beat</th>
            ...
            <tbody>
              {items.map(item => (
                <tr
                  key={item.id}
                  onClick={() => { window.location.href = `/scenes/${item.id}`; }}
