...
export default function DialoguesPage() {
  ...
  const fetchPage = useCallback(async (p: number) => {
    try {
      const res = await fetch(`/api/admin/dialogues?page=${p}&pageSize=${pageSize}`);
      const data = await res.json();
      if (data.success) {
        setItems(data.data.items);
        setTotal(data.data.total);
      } else {
        setError(data.error || 'Failed to fetch dialogues');
      }
    } catch {
  ...
  return (
    <main style={styles.main}>
      ...
      <div style={styles.section}>
        <h2 style={styles.sectionHeading}>Dialogue Trees</h2>
          ...
          <table style={styles.table}>
            <thead>
              <tr>
                ...
                <th style={styles.th}>Node Count</th>
                <th style={styles.th}>Beat Association</th>
            ...
            <tbody>
              {items.map(item => (
                <tr
                  key={item.id}
                  onClick={() => { window.location.href = `/dialogues/${item.id}`; }}
                  style={{ cursor: 'pointer' }}
                >
                  <td style={styles.td}>{item.name}</td>
                  <td style={styles.td}>{item.description?.slice(0, 80)}</td>
                  <td style={styles.td}>{item.nodeCount}</td>
                  <td style={styles.td}>
                    {item.beatAssociation != null
                      ? <span style={{ ...styles.badge, ...styles.infoBadge }}>{item.beatAssociation}</span>
                      : '—'}
                  </td>
